...
import coreRows from "./b2-core.mjs";
import additionalRows from "./b2-additional.mjs";
import finalRows from "./b2-final.mjs";
import completionRows from "./b2-completion.mjs";
...
for (const concept of concepts) {
  if (!concept.key || keys.has(concept.key)) {
    throw new Error(`Missing or duplicate B2 key: ${concept.key}`);
  }
...
await writeFile(join(outDir, "b2.json"), `${JSON.stringify(concepts, null, 2)}\n`, "utf8");
console.log(`Generated ${concepts.length} B2 concepts in ${join(outDir, "b2.json")}`);
